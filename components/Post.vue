<template>
	<div class="w-full bg-white-100 rounded-8px p-10px shadow-card mobile:mb-48px">
		...
		<div class="flex flex-col justify-between min-h-[270px]">
			<div>
				<nuxt-link :to="'/blog/' + post.slug">
					<h3 class="text-24 font-bold mx-10px desktop:mx-16px mb-16px overflow-hidden text-ellipsis" style="display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2;">{{ post.title }}</h3>
				</nuxt-link>
				<p class="text-16 font-light text-grey-80 mb-20px mx-10px desktop:mx-16px overflow-hidden text-ellipsis" style="display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 3;">{{ post.description }}</p>
			</div>
