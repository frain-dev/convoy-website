<template>
    <header>
        <nav class="w-full m-auto px-20px pt-60px pb-20px z-50 fixed left-[50%] -translate-x-1/2 translate-y-0 desktop:pt-50px desktop:pb-12px transition-all duration-300 bg-[#302f3f] shadow-nav backdrop-blur-[36]">
...
const handleScroll = () => {
    route.name == 'blog' || window.scrollY > 50 ? (hasScrolled = true) : (hasScrolled = false);
}
...
//         },
//         handleScroll() {
//             this.$route.name == 'blog' || window.scrollY > 50 ? (hasScrolled = true) : (hasScrolled = false);
//         }
//     },
//     created() {
//         if (process.client) {
