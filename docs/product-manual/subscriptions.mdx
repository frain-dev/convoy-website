...
'twitter:url': https://getconvoy.io/docs/product-manual/subscriptions
---
...
In an Incoming webhooks project, events are routed to a [source](https://www.getconvoy.io/docs/product-manual/sources); then we subscribe multiple endpoints to receive from that source. If no subscriptions are present, events are only saved in the [Event Log](https://www.getconvoy.io/docs/product-manual/events-and-event-deliveries) for reference purposes. If subscriptions are present, we match the event against its subscription filters, only endpoints whose subscription filters match will receive events.
