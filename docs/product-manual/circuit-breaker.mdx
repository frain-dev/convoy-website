...
> - Requires Redis for state storage and a running **agent** (or equivalent process) that executes deliveries so circuit state is applied during dispatch.
...
To update circuit breaker configuration for a specific project:

```bash
convoy circuit-breakers update <project-id> \
