	...
	<div>
		...
		<section class="plans pb-250px px-40px">
			<ul class="grid md-max:gap-40px m-auto max-w-[1200px] -mt-198px type items-center justify-center">
				<li
					class="shadow-default bg-white-100"
					v-for="(plan, index) in plans"
					:key="index"
					:class="`${index == 0 ? 'rounded-tl-16px rounded-bl-16px rounded-br-16px md-max:rounded-tr-16px' : ''} ${
						index == 2 ? 'rounded-tr-16px rounded-bl-16px rounded-br-16px md-max:rounded-tl-16px' : ''
					}`"
				>
					...
					<div class="pt-26px mx-32px pb-42px border-t border-t-grey-20" :class="`${index == 1 ? 'border-none' : ''}`">
						<ul class="ml-20px">
							...
							<li
								class="mb-32px pl-16px"
								:class="`${index == 0 ? 'blue-check' : ''} ${index == 1 ? 'green-check' : ''} ${index == 2 ? 'yellow-check' : ''} `"
